...
async def push_to_arr(db: Session, settings: AppSettings, request: MediaRequest) -> MediaRequest:
    ...
    get_beschaffung(settings).bestand_verwerfen()
    return request
...
async def _tvdb_klaeren(
    ...
    * **Nichts** - dann sagt ``tvdb_id_missing``, woran es liegt, und ob
    ...
    if wahl is not None:
        ...
        if not beschaffung.serien_wahl_erlaubt(zuordnung, wahl):
            raise RequestError(
...
async def cancel(
    ...
    if request.arr_id:
        # Die Fassung der Anfrage entscheidet, wo geloescht wird - sonst bliebe
        # die 4K-Datei liegen, waehrend Nexview "abgebrochen" meldet. Was "schon
        # weg" heisst und was ein echter Fehler ist, entscheidet der Weg.
        try:
