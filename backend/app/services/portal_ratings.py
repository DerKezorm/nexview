...
⚠️ **Warum dieses Modul nicht mehr ``ratings`` heisst.** Genau das war es
einmal - und genau daran ist es gestorben. "Bewertung" heisst bei Nexview
zweierlei: was die **Portale** sagen (hier) und was der **Haushalt** ueber
eine Datei sagt (``services/ratings.py``). Beide lagen unter demselben Namen.

Als in 0.19.0 die Haushalts-Bewertung vom Auftrag auf den Titel umzog, wurde
``ratings.py`` dabei vollstaendig neu geschrieben. Die Portal-Wertungen
verschwanden mit der Datei; der einzige Aufrufer in ``routers/details.py``
blieb stehen und zeigte fortan ins Leere. Ergebnis: **jede Anfrage an
``/api/ratings/movie`` endete mit einem 500**, in 0.19.0 und 0.20.0, und die
Abzeichen fehlten ueberall. Auffallen konnte es nicht - die Oberflaeche zeigt
dann einfach nichts an, und kein einziger Test rief den Endpunkt auf.

Zwei Dinge halten das jetzt zu: der eigene Name, und
``tests/test_portal_ratings.py``, der den Endpunkt wirklich aufruft.
"""
