...
export function AdminStorageSettings() {
  ...
  const speichern = useMutation({
    mutationFn: (werte: { storage_enabled: boolean; storage_default_limit_gb: number }) =>
  ...
  const grenzeGueltig = grenze?.trim() ? Number.isInteger(grenzeZahl) && grenzeZahl > 0 : true
  const geaendert =
      ...
      <Card className="flex flex-col gap-4 p-5">
        ...
        {an && (
          <div className="border-t border-ink-700 pt-4">
            <label className="block text-sm font-medium" htmlFor="storage-default">
